...
window.showLeaderBoard = async () => {
    try {
        ...
        const userLeaderBoardArray = await axios.get(`http://16.171.57.54:3000/premium/showLeaderBoard`, { headers: {"Authorization": token} });
...
window.downloadExpenseFile = () => {
    ...
    axios.get('http://16.171.57.54:3000/expense/download', { headers: { "Authorization": token } })
    .then((response) => {
...
window.addEventListener("DOMContentLoaded", () => {
    if (window.location.pathname === '/FRONTEND/components/Layout/expenses.html') {
        ...
        axios.get(`http://16.171.57.54:3000/expense/get-expense?page=${page}&range=${listRange}`, { headers: {"Authorization": token} })
        .then((response) => {
...
const createExpense = (obj) => {
    const token = localStorage.getItem('token');
    axios.post('http://16.171.57.54:3000/expense/create-expense', obj, { headers: {"Authorization": token} })
        .then((response) => {
...
window.deleteExpense = (expenseId) => {
    const token = localStorage.getItem('token');
    axios.delete(`http://16.171.57.54:3000/expense/delete-expense/${expenseId}`, { headers: {"Authorization": token} })
    .then((response) => {
...
window.getEditExpense = (expenseId) => {
    const token = localStorage.getItem('token');
    axios.get(`http://16.171.57.54:3000/expense/get-edit-expense/${expenseId}`, { headers: {"Authorization": token} }) 
    .then((response) => {
        //console.log(response.data.message);
...
const postEditExpense = (expenseId, obj) => {
    const token = localStorage.getItem('token');
    axios.patch(`http://16.171.57.54:3000/expense/post-edit-expense/${expenseId}`, obj, { headers: {"Authorization": token} })
        .then((response) => {
...
if (premiumBtn) {
    premiumBtn.onclick = async (event) => {
        ...
        try {
            ...
            const response = await axios.get("http://16.171.57.54:3000/purchase/premium-membership", {
            ...
            const options = {
                ...
                handler: async function (paymentResponse) {
                    ...
                    const res = await axios.post(
                        "http://16.171.57.54:3000/purchase/update-transaction-status",
                        {
